...
class main_model(nn.Module):
    ...
    def __init__(self, num_classes, patch_size=4, in_chans=3, embed_dim=96, depths=(1,2,6,2),
                 num_heads=(1,6,12,24), window_size=7, mlp_ratio=4., qkv_bias=True, drop_rate=0,
        ...
        self.stage_task = nn.Sequential(
            nn.Linear(768, 128),
            nn.ReLU(),
            nn.Linear(128, num_classes_stage)
        )

        # 创建程度任务头
        num_classes_severity = 3  # 程度任务的类别数
        self.severity_task = nn.Sequential(
            nn.Linear(768, 128),
            nn.ReLU(),
    ...
    def forward(self, pimgs, eimgs):

        ######  Swin Transformer Branch ######
        x_s, H, W = self.patch_embed(pimgs)
        ...
        ######  ConvNeXt Branch ######
        x_c = self.downsample_layers[0](eimgs)
        ...
        x_all = x_f_4.mean([-2, -1])  # global average pooling (N, C, H, W) -> (N, C)
        x_fu = self.conv_norm(x_all)

        x_stage = self.stage_task(x_fu)
        x_severity = self.severity_task(x_fu)

        return x_stage, x_severity
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, patch_size=4, in_c=3, embed_dim=96, norm_layer=None):
        ...
        self.in_chans = in_c
        self.embed_dim = embed_dim
        self.proj = nn.Conv2d(in_c, 48, kernel_size=patch_size, stride=patch_size)
        self.norm = norm_layer(embed_dim) if norm_layer else nn.Identity()
        self.pool = nn.MaxPool2d(2, stride=2)
        self.down = nn.Conv2d(48, embed_dim, kernel_size=2, stride=2)
    def forward(self, x):
        _, _, H, W = x.shape
